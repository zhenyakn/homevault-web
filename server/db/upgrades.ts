...
import {
  upgrades, upgradeOptions, upgradeItems, upgradeOptionPayments,
  type Upgrade, type UpgradeOption, type UpgradeItem, type UpgradeOptionPayment,
} from "../../drizzle/schema";
import { getDb } from "./client";

export type UpgradeOptionWithPayments = UpgradeOption & { payments: UpgradeOptionPayment[] };
...
async function attachPayments(optionRows: UpgradeOption[]): Promise<UpgradeOptionWithPayments[]> {
...
export async function getUpgradeOptions(upgradeId: string): Promise<UpgradeOptionWithPayments[]> {
  const db = await getDb();
  const rows = await db
    .select()
    .from(upgradeOptions)
...
export async function getUpgradeOptionPayments(optionId: string): Promise<UpgradeOptionPayment[]> {
  const db = await getDb();
  return await db
    .select()
    .from(upgradeOptionPayments)
...
export async function createUpgradeOptionPayment(data: typeof upgradeOptionPayments.$inferInsert): Promise<UpgradeOptionPayment> {
  const db = await getDb();
...
export async function deleteUpgradeOptionPayment(id: string, optionId: string) {
  ...
  await db
    ...
    .where(and(eq(upgradeOptionPayments.id, id), eq(upgradeOptionPayments.optionId, optionId)));
  return true;
}
