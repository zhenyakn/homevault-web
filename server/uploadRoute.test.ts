...
vi.mock("./storage/types", async () => {
  ...
  class StorageOperationError extends Error {
    ...
    backend: any;
    constructor(backend: any, msg: string) {
      super(msg);
...
async function uploadFile(
  url: string,
  init: { mimeType?: string; content?: Buffer; filename?: string } = {}
) {
  const body = new FormData();
  const blob = new Blob([init.content ?? REAL_PNG], {
    type: init.mimeType ?? "image/png",
  });
...
describe("POST /api/upload", () => {
  ...
  it("rejects unrecognised-format payloads (no magic bytes match)", async () => {
    const junk = Buffer.from(
      "absolute garbage that nothing recognises",
      "utf8"
    );
    const res = await uploadFile(app.url, {
      mimeType: "image/png",
  ...
  it("503 with RECONNECT_REQUIRED when the message mentions invalid_grant", async () => {
    ...
    hoisted.fileFlow.uploadAndRegister.mockRejectedValueOnce(
      new StorageNotConfiguredError(
        "Google Drive needs reconnecting (invalid_grant)."
      )
    );
    const res = await uploadFile(app.url);
    expect(res.status).toBe(503);
  ...
  it("507 with DRIVE_QUOTA_EXCEEDED when the backend surfaces a quota error", async () => {
    ...
    hoisted.fileFlow.uploadAndRegister.mockRejectedValueOnce(
      new StorageOperationError(
        "gdrive" as any,
        "DRIVE_QUOTA_EXCEEDED: storage quota has been exceeded"
      )
    );
    const res = await uploadFile(app.url);
