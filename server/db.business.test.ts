...
describe("getOverdueExpenses", () => {
  ...
  it("returns correct shape: id, label, amount, date", () => {
    const expenses = [makeExpense({ date: "2026-04-01", isPaid: false, name: "Water bill", amount: 17500 })];
    const result = getOverdueExpenses(expenses as any, "2026-05-01");
    expect(result[0]).toEqual({ id: "exp-1", label: "Water bill", amount: 17500, date: "2026-04-01" });
  });
  ...
  it("handles mix of paid and unpaid correctly", () => {
    const expenses = [
      ...
      makeExpense({ id: "c", date: "2026-04-01", isPaid: false, isRecurring: false }),
    ];
    const result = getOverdueExpenses(expenses as any, "2026-05-01");
    expect(result).toHaveLength(1);
...
describe("calcMonthlyStats", () => {
  it("sums expenses within the month range", () => {
    ...
    const { monthSpent } = calcMonthlyStats(expenses as any, "2026-04-01", "2026-04-30");
  ...
  it("calculates monthlyRecurring across ALL expenses (not just the window)", () => {
    const expenses = [
      makeExpense({ date: "2026-03-01", amount: 8000, isRecurring: true, recurringInterval: "monthly" }),
      makeExpense({ date: "2026-04-01", amount: 8000, isRecurring: true, recurringInterval: "monthly" }),
    ];
    const { monthlyRecurring } = calcMonthlyStats(expenses as any, "2026-04-01", "2026-04-30");
    expect(monthlyRecurring).toBe(16000);
  });

  it("does not count quarterly/yearly as monthly recurring", () => {
    const expenses = [
      makeExpense({ amount: 5000, isRecurring: true, recurringInterval: "quarterly" }),
      makeExpense({ amount: 3000, isRecurring: true, recurringInterval: "yearly" }),
    ];
    const { monthlyRecurring } = calcMonthlyStats(expenses as any, "2026-04-01", "2026-04-30");
  ...
  it("groups monthCats correctly", () => {
    const expenses = [
      makeExpense({ date: "2026-04-01", amount: 1000, category: "Tax", isRecurring: false }),
      makeExpense({ date: "2026-04-05", amount: 2000, category: "Tax", isRecurring: false }),
      makeExpense({ date: "2026-04-10", amount: 500, category: "Utilities", isRecurring: false }),
    ];
    const { monthCats } = calcMonthlyStats(expenses as any, "2026-04-01", "2026-04-30");
