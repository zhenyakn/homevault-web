...
2. **"Rejects.toThrow" is too broad.** If you only check that _something_ throws, you can't tell whether Zod rejected a value or whether a missing required field triggered an unrelated validation error. For constraint-specific tests, consider asserting on the error message or testing the positive path too.
...
All of these were invisible during the first security pass (P3) because that pass focused on _top-level entity_ update/delete procedures. The four security holes were all in _child entity payment_ paths and one _create_ path — the assumption was that mutations need a `ctx` destructure to do anything harmful, but `protectedProcedure` already ensures the user is authenticated; the missing check was _whose_ records they could touch.
...
2. **"Logged in" is not "authorized."** `protectedProcedure` ensures authentication. Authorization — _whose_ data can this user touch — is a separate, explicit check that must appear in every mutation. It cannot be assumed.
...
2. **The migration runner's comment filter must strip comment lines, not entire chunks.** A chunk is a multi-line SQL statement with optional header comments. Only discard chunks that are _all_ comments.
