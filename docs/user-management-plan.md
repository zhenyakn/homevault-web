...
| **18. Capabilities (feature gating)** ✅ | Code capability registry assigned to plans; mode-aware enforcement | Done — `CAPABILITIES` registry (first: `files.upload`); each plan stores its included capability keys; `hasCapability(tenantId, cap)` = always-true in standalone, plan-gated in SAAS. Enforced on file upload (403 `CAPABILITY_REQUIRED`); admin assigns capabilities per plan via checkboxes. Verified: real-MySQL integration (standalone-included vs SAAS-gated) + build. |
