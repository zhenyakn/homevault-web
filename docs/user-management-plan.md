...
- **Isolation hardening:** ✅ per-tenant storage prefixes (local/S3 key prefix, Drive folder)
  and ✅ per-tenant + per-IP rate limiting now exist. A DB-layer query wrapper that _requires_ a
...
| **19. Standalone / HA add-on safety** ✅        | Pin the add-on to non-SAAS and hide billing UI on single-install                                                                                                                 | Done — `homevault-addon/run.sh` exports `APP_MODE=standalone` (not an add-on option; also blocked by the NO_AUTH↔SAAS guard), so every capability stays included and no SAAS path engages. `billing.capabilities` now returns `isSaas`; the client hides the Plan nav (both shells), the admin **Plans** tab, and renders a "self-hosted — all features included" note on the Plan page when not SAAS. Existing add-on behaviour is unchanged. Verified: integration (`isSaas` standalone vs SAAS) + in-app screenshots (no Plan nav/tab, self-hosted note) + suite/build.                                                                                                                                                                                    |
...
| Risk                                            | Mitigation                                                                                                    |
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| Cross-tenant data leakage during refactor       | Tenant-scope at the DB layer; isolation integration tests; consider a query helper that _requires_ `tenantId` |
