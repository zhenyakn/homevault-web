...
| **Stage 2 — remaining SAAS infra (§4.2)** ◻︎ | Real billing provider + webhooks, custom domains, scaling, observability | Per-tenant rate limiting, GDPR export/deletion, and per-tenant storage isolation are the next in-progress items; a real billing provider, custom domains, scaling, and observability remain deployment/external-service dependent. |
