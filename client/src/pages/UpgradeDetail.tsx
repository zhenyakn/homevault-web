    ...
    >
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-3 pt-1">
          ...
          <Button
            className="w-full"
            disabled={!f.name || create.isPending || update.isPending}
            onClick={submit}
          >
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          <DialogTitle>
            {t("upgradeDetail.logPayment")} — {optionName}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-3 pt-1">
          <div className="space-y-1">
            <Label>{t("upgradeDetail.amountRequired")}</Label>
            <Input
              type="number"
              value={f.amount}
          ...
          <div className="space-y-1">
            ...
            <Input
              value={f.notes}
              onChange={e => setF({ ...f, notes: e.target.value })}
              placeholder={t("upgradeDetail.paymentNotesPlaceholder")}
            />
          </div>
          <div className="space-y-1">
  ...
  return (
    <div
      className={cn(
        ...
        option.selected ? "border-primary" : "border-border"
      )}
    >
      {/* Header */}
      <button
      ...
      >
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {option.selected && (
              <Badge className="bg-green-50 text-green-700 dark:bg-green-950/40 dark:text-green-400 border-0 text-xs h-5">
                {t("common.selected")} ✓
              </Badge>
            )}
          </div>
            ...
            {payments.length > 0 && (
              <div className="space-y-1">
                ...
                {payments.map((p: any) => (
                  <div
                    key={p.id}
                    className="flex items-center justify-between text-xs gap-2 group/payment"
                  >
...
export default function UpgradeDetail() {
  ...
  const [savingStatus, setSavingStatus] = useState<UpgradeStatusStep | null>(
    null
  );
  ...
  if (!upgrade)
    return (
      <div className="p-6 text-center text-muted-foreground">
        {t("upgradeDetail.notFound")}{" "}
        <button className="underline" onClick={() => navigate("/upgrades")}>
          {t("common.back")}
        </button>
      </div>
    );
  ...
  const progress =
    (upgrade.estimatedCost ?? 0) > 0
      ? Math.min(100, (spentAmt / upgrade.estimatedCost!) * 100)
      : 0;
          ...
          {UPGRADE_STATUSES.map((s, idx) => (
            <div key={s} className="flex items-center">
              ...
              {idx < UPGRADE_STATUSES.length - 1 && (
                <div
                  className={cn(
                    "w-8 h-0.5 mx-0.5 mb-3",
                    idx < statusIdx ? "bg-primary" : "bg-border"
                  )}
                />
              )}
            </div>
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          <div>
            ...
            <p className="text-[10px] text-muted-foreground uppercase tracking-wide mt-0.5">
              {t("upgradeDetail.vendors").split(" ")[0]}
            </p>
            {selectedOption && (
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">
              {formatCurrency(
                Math.max(0, (upgrade.estimatedCost ?? 0) - spentAmt)
              )}{" "}
              {t("upgradeDetail.leftLabel")}
            </p>
          </div>
        </div>
        <div className="space-y-1">
          ...
          <div className="flex justify-between text-[10px] text-muted-foreground">
            <span>
              {t("upgradeDetail.budgetUsed", { pct: Math.round(progress) })}
            </span>
            <span>
              {formatCurrency(spentAmt)} {t("dashboard.paid")}
            </span>
          </div>
        </div>
      </div>
        ...
        <section className="space-y-3">
            ...
            <h2 className="text-xs font-bold uppercase tracking-widest text-muted-foreground">
              ...
              {items.length > 0 && (
                <span className="ml-2 font-normal normal-case text-muted-foreground/70">
                  {doneItems.length} {t("common.done").toLowerCase()} ·{" "}
                  {needsAction.length} {t("dashboard.remaining").toLowerCase()}
                </span>
              )}
            </h2>
            <Button
          ...
          {loadingItems ? (
            ...
            <button
              ...
              className="w-full border border-dashed border-border rounded-xl p-4 text-sm text-muted-foreground hover:bg-muted/30 transition-colors flex items-center justify-center gap-2"
            >
