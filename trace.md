...
| Issue | Risk | Status |
|---|---|---|
| `assertOwner` is a post-fetch check, not in WHERE clause | IDOR possible if check logic has a bug | Partial — top-level entities have `ownerId` in WHERE; child entities use lookup+assert chain (unavoidable since they have no `ownerId` column) |
| `repairQuotes.logPayment/deletePayment` — no ownership check | IDOR — any user could log/delete payments on any quote | **Fixed 2026-05-07** |
| `upgradeOptions.logPayment/deletePayment` — no ownership check | IDOR — any user could log/delete payments on any option | **Fixed 2026-05-07** |
| `upgradeItems.create` — no ownership check | Any user could add items to any upgrade | **Fixed 2026-05-07** |
| `calendar.delete` — no ownership check | Any user could delete any calendar event | **Fixed 2026-05-07** |
| No rate limiting | Brute force / DoS | **Fixed 2026-05-07** (P8) |
| ENV vars not validated at startup | Silent misconfiguration | **Fixed 2026-05-07** (P2) |
...
| No structured error logging | Operational blind spot | **Fixed 2026-05-07** (P10) |
...
| Date | Change | Reason |
| 2026-06-07 | Bug fix — Settings tab nav no longer parks the URL on an invalid hash (root cause of the "404 after language change") | `Settings.go()` did `window.history.replaceState(null, "", "#" + id)`, writing a bare hash like `#appearance`. wouter's `useHashLocation` parses that to `/appearance`, which matches no route. Because `replaceState` fires no `hashchange`, wouter didn't re-read it at click time, so the page looked fine — but `useSyncExternalStore` re-reads the hash on every render, so the next global re-render (a language change via `i18n.changeLanguage`, or an apartment switch's reload) surfaced the stale hash and rendered the NotFound 404 page. Fixed by navigating through the hash router: `setLocation("/settings/" + id, { replace: true })`, which yields a valid `#/settings/<id>` route and keeps wouter's store consistent. |
| 2026-06-07 | Bug fix — switching property no longer 404s on detail pages | `PropertyContext.switchProperty` did `window.location.reload()` in place. On a detail route (`/repairs/:id`, `/upgrades/:id`) the reload kept the hash, refetched the new property's list (which lacks that id) and stranded the user on a dead-end "not found" page. Now it leaves any detail route for its parent list and refetches via `queryClient.invalidateQueries()` instead of a full reload (the `x-property-id` header is read from localStorage per request), avoiding both the 404 and the white flash. `RepairDetail`/`UpgradeDetail` also auto-recover to their list when the id is absent after load, instead of rendering a 404. |
| 2026-05-07 | Security — `upgradeOptions.logPayment` and `.deletePayment` now require ownership | Same gap as repairQuotes payment endpoints. Added `getUpgradeOptionById` lookup + `assertUpgradeOwner` chain to both. |
| 2026-05-07 | Security — `upgradeItems.create` now requires ownership of parent upgrade | The create path had no `ctx` access and no ownership check. Any authenticated user could add items to any upgrade by knowing its ID. Added `assertUpgradeOwner` before insert. |
| 2026-05-07 | Security — `calendar.delete` now requires ownership | `deleteCalendarEvent` accepted only `id` and had no owner check. Added `ownerId` parameter to `db.deleteCalendarEvent` and baked it into the WHERE clause. Router now passes `ctx.user.id`. |
| 2026-05-07 | Data integrity — `loans.addRepayment` keeps `currentBalance` in sync | `currentBalance` in the DB was set at creation and never updated when a repayment was logged. `buildLoanSummary` computed the real balance from the repayments array (correct), but `currentBalance` drifted silently. `addRepayment` now computes `max(0, originalAmount - totalRepaid)` and writes it atomically with the repayments update. Also removed stray `ownerId` field from repayment objects (never belonged there). |
| 2026-05-07 | Bug fix — `property.delete` hardcoded ID 1 as the undeletable property | Guard was `if (propertyId === 1)` — correct only by coincidence for a single-user install where the first property happens to be ID 1. Replaced with `if (props.length <= 1)` (can't delete your only property), which is the correct semantic. |
| 2026-05-07 | Performance — `context.ts` property ownership check is now a targeted query | On every authenticated tRPC request, `createContext` was calling `getPropertiesByUser(userId)` (full list) just to validate the `x-property-id` header. Replaced with `checkPropertyOwnership(userId, propertyId)` (single `SELECT id` with 2-column WHERE). The full list query is only issued on the fallback path (spoofed or stale header), which is rare. |
|---|---|---|
...
| 2026-05-07 | HA add-on hotfix v0.2.1 — complete v1→v2 convergence in migration script | Live HA add-on failed with `Unknown column 'name' in 'INSERT INTO'` on expenses INSERT after upgrade. Root cause: the convergence section covered only post-v2 additions (isPaid, repayments…) but NOT the v1→v2 column renames from migration 0007 (label→name in expenses/loans/wishlistItems/purchaseCosts, label→title in repairs/upgrades, contractorName→contractor/quotedPrice→amount/isSelected→selected in repairQuotes, name→title/totalPrice→estimatedCost/isSelected→selected in upgradeOptions, vendorName→store in upgradeItems). Rewrote convergence section: 50+ ADD COLUMN statements covering all 10 affected tables, no backfills (no real data in existing installs). Tagged v0.2.1 to trigger new Docker build. Static analysis tests (addon.test.ts) pass 288/288. |
| 2026-05-07 | HA add-on hotfix v0.2.2 — loans.attachments and wishlistItems.attachments missing from convergence | Dashboard SELECT failed: `loans.attachments` was added to schema.ts and the CREATE TABLE block but never via any drizzle migration or ALTER TABLE convergence entry — so existing HA databases never had the column. The seed's loans INSERT also uses `attachments: []`, causing a cascade failure that left wishlist/purchaseCosts/calendar/inventory tables empty. Added both `loans.attachments` and `wishlistItems.attachments` to the convergence section. Tagged v0.2.2. |
| 2026-05-07 | HA add-on hotfix v0.2.3 — Phase 1 v1 schema reset replaces MODIFY COLUMN approach | Seed continued failing: `Field 'label' doesn't have a default value`. Migration 0008 made all v1 NOT NULL columns nullable via MODIFY COLUMN, but that was never reflected in the unified HA migration script. Rather than enumerating dozens of MODIFY COLUMN statements, introduced `dropIfLegacyV1(table, canaryColumn)` helper: queries information_schema for a canary that is still NOT NULL, drops the table if found, then CREATE TABLE IF NOT EXISTS recreates it with the correct v2 schema. Covered all 10 tables from migration 0008 (expenses/repairs/repairQuotes/upgrades/upgradeOptions/loans/wishlistItems/purchaseCosts/calendarEvents). Safe because no real data in HA installs; fires exactly once per table. Tagged v0.2.3. |
| 2026-05-07 | HA add-on hotfix v0.2.4 — upgradeItems missing from Phase 1 reset list | Seed failed on `upgradeItems` INSERT: `Field 'ownerId' doesn't have a default value`. upgradeItems.ownerId was NOT NULL in v1 (removed from v2 schema by migration 0008) but was the only table from 0008 not included in the dropIfLegacyV1 calls. Added `dropIfLegacyV1("upgradeItems", "ownerId")` to complete the set. All 10 tables from 0008 now covered. Tagged v0.2.4. |
| 2026-05-07 | Architectural — dashboard rewritten with SQL aggregates instead of full-table loads | `getDashboardStats` previously loaded every expense, repair, upgrade, and loan into Node.js memory for filtering. Replaced with 10-way `Promise.all` using SQL `COALESCE(SUM(...),0)`, `COUNT(*)`, and `GROUP BY category`. Stale repairs use a JS Date cutoff with Drizzle's `lte`. Open repairs sorted in DB via `CASE WHEN priority`. Loan summary now reads `currentBalance` directly (kept in sync by `addRepayment`). `getPortfolioSummary` similarly rewritten. Dashboard module no longer imports `parseJsonArray`. |
| 2026-05-07 | Tests — `server/migrate.test.ts` added (38 tests) | Static-analysis tests for the migration runner and SQL files: parser unit tests (leading-comment handling), MySQL 8.4 syntax compatibility (no `IF EXISTS` / `IF NOT EXISTS` on ALTER/INDEX DDL), statement count guard for `0010_payment_tables.sql`, structural coverage for payment tables and ordering invariants. Tests immediately found pre-existing issues in `0004_settings_columns.sql` (`ADD COLUMN IF NOT EXISTS`) and `0005_loan_repayments_and_wishlist_propertyid.sql` (`CREATE INDEX IF NOT EXISTS`), and a schema drift bug: `loanRepayments.ownerId NOT NULL` from 0005 was never cleaned up by 0010. All fixed. |
| 2026-05-07 | Bug fix — `scripts/migrate.ts` silently dropped SQL chunks with leading comments | The parser split on `statement-breakpoint` then filtered any chunk whose first non-whitespace character was `--`. A chunk like `-- comment\nCREATE TABLE ...` would be silently discarded. Fixed: strip leading comment lines from each chunk before filtering, so only pure-comment chunks are skipped. Also: `DROP COLUMN IF EXISTS` is not supported on MySQL 8.4 Windows — changed to plain `DROP COLUMN`; runner's IGNORABLE set already handles `ER_CANT_DROP_FIELD_OR_KEY`. Same fix applied to `apply-migration-addon.mjs`. |
| 2026-05-07 | Architectural — payment tables migration: JSON arrays → relational tables | `repairQuotes.payments`, `upgradeOptions.payments`, and `loans.repayments` were JSON arrays stored in parent-table columns. Replaced with three relational tables: `repairQuotePayments`, `upgradeOptionPayments`, `loanRepayments` — each with FK + `ON DELETE CASCADE`. `server/db/repairs.ts`, `upgrades.ts`, `loans.ts` rewritten: `attachPayments()` / `attachRepayments()` batch-fetches child rows in one query then maps in JS (avoids N+1). Router updated: `logPayment`/`deletePayment` call the new DB functions; `deletePayment` input changed from `paymentIndex: number` to `paymentId: string` (index-based deletion is semantically wrong for a relational table). Client pages `RepairDetail.tsx` and `UpgradeDetail.tsx` updated accordingly. `drizzle/0010_payment_tables.sql` migration created using JSON_TABLE for data copy. `apply-migration-addon.mjs` updated: Phase 2 CREATE TABLE blocks remove old JSON columns and add 3 new payment tables; Phase 3 removes now-unnecessary ADD COLUMN entries for payments/repayments; Phase 4 added for best-effort JSON→relational data copy (try/catch for MariaDB) + `DROP COLUMN IF EXISTS`. `server/db/seed.ts` updated: loan repayments inserted separately via `loanRepayments`; upgradeOption payments inserted separately via `upgradeOptionPayments`. 308/308 tests pass. |
| 2026-05-07 | i18n audit — missing locale keys and hardcoded strings fixed | Found `upgradeDetail.purchased` and `upgradeDetail.pending` referenced via `t()` in `UpgradeDetail.tsx` but absent from both `en.json` and `he.json` — they rendered as raw key strings in the UI. Added to both locale files. Also audited `RepairDetail.tsx`: 10 hardcoded English strings (toast messages, "Repair not found", "Progress" label) were bypassing the translation system entirely. Added corresponding keys to `repairDetail.*` in both locale files and replaced all hardcoded strings with `t()` calls. |
| 2026-05-11 | UI redesign — green brand theme, grouped nav, desktop topbar, bento dashboard | `index.css`: primary color changed from blue (`color-blue-700`) to forest green (`oklch(0.480 0.113 155)`, ≈#2B7A55); sidebar accent updated to green-tint; chart colors updated to green shades; dark-mode variants updated to match. `DashboardLayout.tsx`: logo mark changed from violet/indigo gradient to solid green (`bg-primary`); navigation restructured from flat list to three groups — Overview (Dashboard, Calendar), Finances (Expenses, Loans, Purchase Costs), Property (Repairs, Upgrades, Inventory, Wishlist), Account (Settings); desktop topbar added inside `SidebarInset` showing breadcrumb (Section › Page) and a search pill (⌘K); compact `ThemeToggle` icon added to topbar alongside the existing full toggle in sidebar footer; mobile topbar uses `pageMeta` lookup for correct page title. `Dashboard.tsx`: replaced vertical section layout with 12-column bento grid — Row 1: SpendCard (5-col) + LoansCard (4-col) + OpenItemsCard (3-col); Row 2: AttentionCard (8-col) + CalendarCard (4-col); Row 3: UpgradesCard (12-col, only when active upgrades exist). AttentionCard redesigned from grid-of-cards to list-of-rows inside a single card. CalendarCard redesigned with date-box style. New `OpenItemsCard` shows counts for urgent repairs, high-priority repairs, active upgrades, overdue bills. `nav.group.*` keys added to en.json and he.json. Seven new `dashboard.*` locale keys added (openItems, urgentRepairs, highPriority, overdueBills, upcoming, totalOutstanding, itemsNeedAction). 346/346 tests pass. |
| 2026-05-11 | Mock data — `mockRepairs` rewritten to exercise the full repair domain (8 entries with embedded quotes/payments) | The previous 6 entries were flat — no rows in `repairQuotes` / `repairQuotePayments` were ever seeded, despite repairs supporting multi-vendor quotes since v2. Coverage gaps in seeded data: no `Electrical` / `Appliance` / `Other` categories, no `urgent` priority, no `waiting_for_contractor` or `cancelled` status. Rewrote `mockRepairs` to 8 scenarios mirroring how `mockUpgrades` carries `options[]`/`payments[]`: added `SeedRepair` type wrapping `InsertRepair` with optional `quotes[]` (each with optional `payments[]`). New scenarios cover every enum value: all 6 statuses, all 4 priorities, all 7 categories. Realistic Tel Aviv apartment context (Avi Plumbing, Eli Gabai electrician, Roni Waterproofing, va'ad bayit, Samsung extended warranty, etc.). `server/db/seed.ts`: imported `repairQuotePayments`; added pre-cleanup that deletes existing `repairQuotes` (cascades to payments via FK) before deleting repairs — necessary because the FK `repairQuotes.repairId → repairs.id` is not `ON DELETE CASCADE`, so the existing seed would have orphaned quote rows once any were seeded. Replaced flat `db.insert(repairs).values(...)` with per-repair loop that inserts the repair, then `repairQuotes`, then `repairQuotePayments` — mirroring the existing upgrade seed pattern (options + optionPayments). 346/346 tests pass. |
