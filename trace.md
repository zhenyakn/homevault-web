...
| Date | Change | Reason |
| 2026-05-07 | Security — `repairQuotes.logPayment` and `.deletePayment` now require ownership | Neither mutation verified that the caller owned the parent repair. Added `getRepairQuoteById` lookup + `assertRepairOwner` chain to both. |
