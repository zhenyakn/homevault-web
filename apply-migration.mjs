...
async function run() {
  ...
  const statements = [
    ...
    // Foreign keys
    `ALTER TABLE \`calendarEvents\` ADD CONSTRAINT \`calendarEvents_createdById_users_id_fk\` FOREIGN KEY (\`createdById\`) REFERENCES \`users\`(\`id\`) ON DELETE no action ON UPDATE no action`,
    ...
    `ALTER TABLE \`wishlistItems\` ADD CONSTRAINT \`wishlistItems_ownerId_users_id_fk\` FOREIGN KEY (\`ownerId\`) REFERENCES \`users\`(\`id\`) ON DELETE no action ON UPDATE no action`,
    // Indexes
    `CREATE INDEX \`calendar_date_idx\` ON \`calendarEvents\` (\`date\`)`,
    `CREATE INDEX \`calendar_created_by_idx\` ON \`calendarEvents\` (\`createdById\`)`,
    ...
    `CREATE INDEX \`wishlist_priority_idx\` ON \`wishlistItems\` (\`priority\`)`,
    `CREATE INDEX \`wishlist_owner_idx\` ON \`wishlistItems\` (\`ownerId\`)`,
  ];

  // Also insert default property row
  const insertDefault = `INSERT IGNORE INTO \`properties\` (\`id\`) VALUES (1)`;
  ...
  for (const sql of statements) {
    try {
      ...
      if (e.code === 'ER_DUP_KEYNAME' || e.code === 'ER_FK_DUP_NAME') {
        ...
        console.log('ERROR:', e.message, '| SQL:', sql.substring(0, 60) + '...');
      }
    }
  }

  try {
    await conn.execute(insertDefault);
    console.log('OK: Default property row inserted');
  } catch (e) {
    console.log('Default property:', e.message);
  }
